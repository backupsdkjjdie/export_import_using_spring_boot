...
public class ExportXlsView {
	...
	private <T> void writeHeader(List<T> data) {
		try {
			...
			if (data.get(0).getClass().getSuperclass() != null) {
				Map<String, ExcelValue> fieldNamesSuper = getFieldNamesForClass(data.get(0).getClass().getSuperclass());
				fieldNames.putAll(fieldNamesSuper);
			}
